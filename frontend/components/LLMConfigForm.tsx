...
interface Props {
  onSave: (data: { provider_name: string; api_base: string; api_key: string; model_name: string }) => Promise<void>
  initial?: { provider_name: string; api_base: string; api_key: string; model_name: string }
}
...
export function LLMConfigForm({ onSave, initial }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await onSave({ provider_name: providerName, api_base: apiBase, api_key: apiKey, model_name: modelName })
    } finally {
      setSaving(false)
    }
  }
